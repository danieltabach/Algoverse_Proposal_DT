...
flowchart LR
    ...
    subgraph ARMB["Arm B — Attractor Susceptibility"]
        C1["Self-talk dyads: 2 instances,<br/>System Card seeds, 30 turns x ~50 seeds"]
        C2["PETRI-seeded audits (stretch)"]
    end
    ...
    subgraph SCORE["Scoring Layer"]
        ...
        S5["Degeneration flag<br/>(repetition-loop detector)"]
    end
...
|---|---|---|---|
| `trait_score` (Y, Arm A) | Position on reductionism↔mystery axis | Pairwise judge votes (both orders, 2 judge families) → Bradley-Terry → normalized | 0–1 |
| `P_entry` (Y, Arm B) | P(bliss-register entry within 30 turns) | Judge threshold crossing, ≥3 consecutive turns, over ~50 runs | 0–1 |
| `turn_of_entry` (Y2, Arm B) | Speed of attractor pull | First turn of sustained crossing | 1–30 / censored |
...
| `lexicon_rate` | Transcendence-vocabulary frequency (judge-free check) | Word counts per 1k tokens (System Card Table 5.5.1.B precedent) | ≥0 |
| `degeneration_flag` | Repetition-loop / incoherence marker | n-gram repetition detector + judge anchor | binary |
...
- Arm B: logistic regression of entry on capability; Kaplan-Meier-style curves for turn-of-entry (censoring at 30).
- Per-family curves alongside pooled — divergence = lab fingerprint finding.

## 5. Scale of the experiment (feasibility)
...
| Arm B calls | 50 seeds × 30 turns × 2 instances × ~15 models ≈ 45k messages | the compute-heavy arm; run after Arm A validates |
...
## 6. Target figures (mock versions in `figures/`)
...
| **Fig 3** `fig3_attractor_susceptibility` | P(bliss entry) vs. PC1 with logistic fit; Claude points annotated | Rising curve = capability-dependent attractor (headline result); Claude-only outlier = lab-specific phenomenon (also a headline) |
...
Supplementary: per-family PC1-vs-log-compute lines (Ruan Fig. 3 replication for our model set); lexicon-rate vs. judge-score correlation (triangulation); turn-of-entry survival curves.
...
5. **Week 6–9:** Arm B on the validated subset → Fig 3.
6. **Week 9–12:** analysis, ablations, paper. (Arm B is droppable if slipping; Arm A alone is a complete paper.)
